...
encarga de borrarlo así como también antes cambiar su next y prev*/
{
    ...
    //Se encarga de verificar que del_node no sea el último nodo(mi pregunta es qu'e pasa si es el último nodo? respuesta=no entra a este if pero si al 4°
    ...
    //Se encarga de verificar que del_node no sea el primer nodo(pues pa' ello ya tenemos el segundo if de esta función, si ese 2° if tuviera return ¿ya no tendriamos este if?)
